...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!guest) return NextResponse.json({ success: false, message: 'Not found' }, { status: 404 })
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        totalPaid,
        payments: guest.payments.map(p => ({ ...p, paymentDate: p.paymentDate.toISOString() })),
      },
    })
  } catch (error) {
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const whatsappLink = getCheckOutLink(guestForLink, totalPaid)
